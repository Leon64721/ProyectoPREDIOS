...
## 18. Sprint 5: Gestión de Equipos y Jerarquía RBAC — [COMPLETADO AL 100% 2026-08-18]
...
**Fase A (continuación 2026-08-18) — Sincronización de Grupos Google + Token Set Matching:** [COMPLETADA]
- `homologacion_usuarios.js`: nueva `sincronizarGruposGoogleIDU()` — enriquece `USUARIOS` desde `dtdp@idu.gov.co`/`stap@idu.gov.co`/`stgsv@idu.gov.co` vía **Admin SDK Directory API** (`AdminDirectory.Members.list()` + `Users.get()`), upsert no destructivo por EMAIL (nunca sobreescribe NOMBRE/COMPONENTE ya curados a mano). `homologarUsuariosMatriz()` mejorada con Token Set Ratio (algoritmo `fuzzywuzzy.token_set_ratio`) combinado con Levenshtein — resuelve nombres informales ("Juan Pérez" → "Juan Carlos Pérez Rodríguez", puntaje 1.0) sin generar falsos positivos entre nombres no relacionados (0.308 en la prueba). Ver detalle completo y verificación en `DOCUMENTACION_TECNICA_VIVA.md` Sección 20.
- **Bloqueo técnico real:** las URLs de `/members` de Google Groups pedidas originalmente requieren sesión de navegador — no son accesibles por `UrlFetchApp`. Se confirmó explícitamente con el usuario (vía `AskUserQuestion`) usar Admin SDK Directory API en su lugar; añadidos el servicio avanzado `AdminDirectory` y 2 scopes OAuth nuevos en `appsscript.json`.
- **⚠️ Pendiente antes de operar `sincronizarGruposGoogleIDU()` en producción:** requiere habilitar "Admin SDK API" en el proyecto de Google Cloud vinculado + que la cuenta ejecutora tenga privilegios de administrador de Grupos en Workspace idu.gov.co — acción de administración externa al código, no ejecutable por un agente. Hasta entonces la función falla con error explícito (`AdminDirectory no está habilitado`), sin romper `homologarUsuariosMatriz()` (desacoplada deliberadamente).

**Fase B — UI, Tablero de Carga y Asignación Granular:** [COMPLETADA 2026-08-18]
- `app_equipos_js.html` (nuevo): `#moduloGestionEquipos` (dentro de `#page-equipos`, patrón `showPage()` del resto de la app) con tarjetas KPI (Total RTs, RTs por Asignar) y tablas de distribución por Articulador/Gestor, alimentadas por `getEstadisticasCargaEquipos()` (nueva, en `gestion_equipos_backend.js` — no estaba en Fase A). Modal `#modalHomologacionEquipos` (dinámico, sin scriptlets GAS — Directiva 2) sobre `detectarUsuariosHuerfanos()`, con acción "Vincular" que reutiliza `reasignarUsuarioMasivo()` de Fase A (reemplaza el nombre libre por el email elegido en todas las filas donde aparece — sin backend nuevo). Formulario de Asignación Granular (`#modalAsignacionGranular`) con cascada Proyecto→Tramo→RT sobre `window.currentData` (mismo patrón que `onProyectoChange()`/`onTramoChange()` de `app_matriz_js.html`) conectado a `asignarEquipoGranular()`. Modal Handover (`#modalHandoverEquipos`) conectado a `reasignarUsuarioMasivo()`. Caché de estado en IndexedDB (`equipos-cache-v1`, clave `equipos_cache_<CURRENT_USER_EMAIL>` — Directiva 1) para pintado optimista de KPIs.
- `Index.html`: item de navegación `#navEquipos` oculto por defecto (`d-none`), mostrado por JS solo para `currentRole` en `{Administrador, Articulador, Gestor}` — Editor/Lector no forman parte de esta jerarquía de negocio, quedan fuera a propósito.

**Fase C — Enrutamiento de Alertas e Integración de Permisos por Rol:** [COMPLETADA 2026-08-18]
- `evaluador_alertas.js`: `evaluarAlertasDataset()` (el path que realmente alimenta el dashboard en vivo — badges, `#modalDetalleAlertas`, PDF de Fase B Sprint 4) y `MotorEvaluadorReglas._registrarAlerta()` (el path que escribe `ALERTAS_ACTIVAS`) ahora resuelven `ARTICULADOR_EMAIL`/`GESTOR_EMAIL` directamente del registro evaluado, vía un chequeo barato de formato de email (`_resolverEmailDesdeCeldaAlertas()`) — sin re-ejecutar homologación difusa por alerta. Una celda que todavía tiene el nombre libre histórico (no migrado por Fase A/B) da `SIN_RESPONSABLE: true` en vez de enrutar a nadie, tal como estaba planificado en `ARCHITECTURE_V4.md` Sección 2.A.
- `gestion_equipos_backend.js`: nueva `getEstadisticasCargaEquipos(userContext)` — el parámetro es solo un hint, el rol/email real que decide el recorte se resuelve siempre server-side (nunca se confía en un rol declarado por el cliente).
- **RBAC de visibilidad — implementado en `Codigo.js` `getDashboardData()`, NO en `app_core_js.html`/`datos.js` como pedía el prompt literal.** Decisión deliberada: un filtro solo-cliente es trivialmente evitable llamando `google.script.run` directo desde devtools; la única forma real de proteger los datos es recortarlos en el servidor antes de que salgan. Administrador/Editor/Lector conservan el comportamiento histórico (sin recorte adicional). Articulador ve solo filas donde `ARTICULADOR JUIRIDICO === su email`; Gestor ve sus propias filas más las de el/los Articulador(es) bajo los que él mismo aparece como Gestor (derivado de `Datos` en una pre-pasada en memoria, sin tabla de reporte separada).
- **Bug de seguridad preexistente encontrado y corregido en el mismo cambio:** el caché de `getDashboardData()` (`CACHE_KEY_DASHBOARD` en `cache_backend.js`) es una clave **fija compartida por todos los usuarios** (documentado ahí mismo: "clave fija"). Sin corrección, el nuevo filtro RBAC se habría calculado bien pero nunca se habría ejecutado si otro usuario ya dejó una respuesta completa en caché — un Articulador podía ver el dataset completo de un Admin durante los 30 minutos de TTL, o a la inversa, un Admin podía recibir por error la vista recortada de un Articulador. Articulador/Gestor ahora se excluyen explícitamente del caché compartido (ni leen ni escriben en él), a costa de recalcular en cada carga — el resto de roles no pierde el beneficio de caché.

**Antipatrones a evitar (explícitos del brief):** asignación parcial sin log, mutación directa de `Datos` sin `LockService`, `localStorage`, renderizado bloqueante del tablero de carga. Ninguno introducido — `app_equipos_js.html` usa `google.script.run` async en todos los flujos, cero DOM bloqueante.

**Resuelto 2026-08-18 (6 de 6 preguntas de arquitectura, confirmadas por el usuario):** `USUARIOS` ya existe en spreadsheet separado (URL en `ARCHITECTURE_V4.md` Sección 6.1) y reemplaza/deprecra `PAC_Articuladores`; Articulador/Gestor se agregan a `CONFIG.ROLES`; 1 RT → 1 Gestor exclusivo; `LOGS_ASIGNACION` va en spreadsheet nuevo y dedicado; esquema de `USUARIOS` confirmado; columnas en `Datos` confirmadas (`ARTICULADOR JUIRIDICO` — typo real de producción — y `GESTOR JURÍDICO`).

**Context:** ver `ARCHITECTURE_V4.md` completo y `DOCUMENTACION_TECNICA_VIVA.md` Secciones 19-21 para el diseño técnico, el grounding verificado contra código real, y la evidencia de cada fase (`node --check`, `clasp push`, hashes de commit).

**Pendientes reales para operación en producción (ninguno bloquea el código ya desplegado):**
1. `CONFIG.DATA_FILES.LOGS_ASIGNACION` sigue en placeholder — crear el spreadsheet dedicado y reemplazar el ID (auditoría de asignaciones deshabilitada hasta entonces, las asignaciones en sí ya funcionan).
2. `sincronizarGruposGoogleIDU()` requiere habilitar "Admin SDK API" en Cloud Console + privilegios de administrador de Grupos en Workspace para la cuenta ejecutora — acción externa al código.
3. Validación visual en runtime de los 3 flujos nuevos (KPIs, homologación, asignación/handover) con navegador autenticado — no ejecutada por un agente de código, mismo patrón que el ítem 13 de este documento desde Sprint 1.

**Depends on / blocked by:** ninguno funcional para el código desplegado. Los 3 pendientes de arriba son de configuración externa/validación humana, no de desarrollo.
