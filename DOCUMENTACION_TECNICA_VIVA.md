...
**Addendum 2026-08-18 [CONC-BE-08] — diagnóstico más preciso del pendiente de Admin SDK:** los logs de producción confirmaron que `typeof AdminDirectory !== 'undefined'` (el servicio avanzado sí está habilitado en Apps Script), pero `AdminDirectory.Members` es `undefined` en ejecución — es decir, activar el servicio avanzado en el editor de Apps Script **no fue suficiente**; falta habilitar "Admin SDK API" específicamente en el proyecto de Google Cloud Console vinculado al script. Se añadió un guardarraíl explícito en `_obtenerDirectorioGruposIDU()` (`console.warn` con mensaje accionable, degrada a `[]` sin romper la homologación) y en `sincronizarGruposGoogleIDU()` (lanza `Error` descriptivo antes de iniciar la paginación, en vez de fallar a mitad de la llamada a `Members.list()` con un error genérico de Apps Script). Commit `ca9b2f6`. `node --check` OK. `npx clasp push --force` → `Pushed 49 files`. Sigue sin resolverse el pendiente en sí — este cambio solo hace el diagnóstico visible y accionable para quien tenga acceso a Cloud Console.

## 23. Sprint 5 — Sembrado manual de usuarios DTDP/STAP en USUARIOS — [CÓDIGO DESPLEGADO 2026-08-18, EJECUCIÓN PENDIENTE]

**Agente:** Claude Code (Claude Sonnet 5, orquestado con gstack v1.60.1.0).

**Origen:** mientras el pendiente de Admin SDK API (Secciones 20 y 22) sigue sin resolverse, el usuario auditó manualmente los grupos `dtdp@idu.gov.co`/`stap@idu.gov.co` (exportación directa desde la consola de Grupos de Google) y compartió el inventario resultante para sembrarlo en `USUARIOS` sin depender de la API.

**Discrepancia encontrada y corregida antes de desplegar:** el usuario reportó "266 cuentas únicas". El array recibido, verificado dos veces de forma independiente (`eval` sobre el literal + `grep -c "{ email:"`), tiene **260 registros únicos**, no 266. Las membresías por grupo sí cuadran exactamente con lo reportado (DTDP: 215, STAP: 175 — la diferencia con 260 personas es el solapamiento de quienes están en ambos grupos, resuelto en el campo `componente`). Sin duplicados de email, sin emails con formato inválido, sin nombres vacíos. Se corrigió la cifra en el docstring del archivo y en el mensaje de commit (`260`, no `266`) en vez de reproducir un número no verificado — no se inventaron los 6 registros faltantes para cuadrar la cifra original.

**Cambios realizados:**
- **Creado `sembrado_usuarios_grupos.js`:** `DIRECTORIO_OFICIAL_SEMBRADO` (260 objetos `{email, nombre, componente}`) + `ejecutarSembradoInicialUsuarios()` — upsert atómico por `EMAIL` contra `USUARIOS` bajo `LockService` (60s, mismo timeout que `gestion_equipos_backend.js`), con una sola `setValues()` por columna modificada (Directiva 3, mismo patrón que `sincronizarGruposGoogleIDU()`). Al finalizar el upsert, invoca automáticamente `homologarUsuariosMatriz()` para recalcular la homologación sobre `Datos` con el directorio recién enriquecido, sin que el usuario tenga que disparar un segundo paso manual.

**Verificación ejecutada:** `node --check` sobre `sembrado_usuarios_grupos.js`, `homologacion_usuarios.js`, `gestion_equipos_backend.js` — 3/3 OK. Validación de datos en Node (conteo, duplicados, formato de email, nombres vacíos, totales de membresía) antes de comitear.

**Evidencia de despliegue:** `npx clasp push --force` → `Pushed 50 files`, incluyendo `sembrado_usuarios_grupos.js` como archivo nuevo.

**IMPORTANTE — lo que NO se pudo hacer desde este entorno de agente:** el prompt pedía "desplegar e invocar" la función. Solo se pudo desplegar el código. Se intentó `npx clasp run ejecutarSembradoInicialUsuarios` para invocarla remotemente — falla con `"Script function not found. Please make sure script is deployed as API executable"`, porque el proyecto está desplegado como Web App, no como API Executable (crear un deployment de ese tipo es un cambio de configuración de despliegue con implicaciones de acceso que no se tomó unilateralmente sin autorización explícita del usuario). **`USUARIOS` todavía NO tiene los 260 registros sembrados** — el código está listo pero nadie lo ha ejecutado todavía. Instrucciones de ejecución manual entregadas al usuario en el chat de esta sesión.

**Depends on / blocked by:** ejecución manual pendiente — alguien con acceso al editor de Apps Script debe correr `ejecutarSembradoInicialUsuarios()` una vez (Editor → seleccionar la función en el desplegable → Ejecutar) para que el sembrado y la re-homologación tengan efecto real en `USUARIOS`/`Datos`.

## 24. Sprint 5 — Carga de Línea Cero (Baseline) del módulo de equipos — [DESPLEGADO 2026-08-18, EJECUCIÓN PENDIENTE]

**Agente:** Claude Code (Claude Sonnet 5, orquestado con gstack v1.60.1.0).

**Origen:** el usuario aclaró que el objetivo no es un "find and replace" cosmético, sino fijar la "Línea Cero" — la distribución REAL vigente de quién tiene qué RT hoy (leída de `Datos`), homologada a email oficial, como punto de partida operativo del módulo de equipos.

**Cambios realizados:**
- **`homologacion_usuarios.js` — `obtenerMapeoLineaCero()` (nueva):** llama a `homologarUsuariosMatriz()` y arma `{nombreViejo → emailOficial}` solo para `ENCONTRADO_ACTIVO`, `ENCONTRADO_SIN_PERFIL`, o `SIMILITUD_APROXIMADA` con puntaje ≥ `LINEA_CERO_UMBRAL_APROXIMADA = 0.85` — más estricto que el `0.75` general (`HOMOLOGACION_ENGINE.umbralPuntaje`), porque ese umbral es para sugerir a un humano en el modal (con revisión), y este alimenta una escritura automática y masiva sin revisión fila por fila. Deja fuera `ENCONTRADO_INACTIVO` (no se debe asignar RTs a alguien inactivo) y `NO_ENCONTRADO`.
- **`gestion_equipos_backend.js` — `ejecutarCargaLineaCero()` (nueva):** bajo `LockService` (60s), lee las columnas `ARTICULADOR JUIRIDICO`/`GESTOR JURÍDICO` completas una vez, reemplaza en memoria por lotes de 1000 cada nombre libre presente en el mapeo por su email oficial, y escribe con como máximo 2 `setValues()` (una por columna, solo si esa columna tuvo cambios reales). Registra **un solo evento resumen** en `LOGS_ASIGNACION` ("Inicialización de Línea Cero (Baseline) completada — N asociaciones...") en vez de una fila por RT afectado — decisión explícita del usuario, evita inundar el log con miles de filas en una sola operación masiva. Invalida el caché de `getDashboardData()` al terminar para que el árbol y el RBAC reflejen la migración de inmediato.
- **`app_equipos_js.html` / `Index.html`:** botón `Cargar Línea Cero (Asignación Inicial)` en la cabecera de `#moduloGestionEquipos`, con confirmación explícita (reutiliza `showConfirmation()`/`pendingAction` ya existente en `app_core_js.html`, mismo patrón que `logout()`/`deletePermission()` — no se introdujo un `confirm()` nativo). 100% asíncrono vía `google.script.run`.

**Verificación ejecutada:** `node --check` sobre `homologacion_usuarios.js` y `gestion_equipos_backend.js`; extracción + `node --check` de `app_equipos_js.html` — 3/3 OK. Prueba aislada en Node de la lógica de umbral/confianza (6 casos: ACTIVO, SIN_PERFIL, INACTIVO, APROXIMADA≥0.85, APROXIMADA<0.85, NO_ENCONTRADO) — 6/6 correctas.

**Evidencia de despliegue:** `npx clasp push --force` → `Pushed 50 files`.

**IMPORTANTE:** igual que en la Sección 23, `ejecutarCargaLineaCero()` está desplegada pero **no se ha ejecutado** — este entorno de agente no puede invocar funciones de Apps Script remotamente (`clasp run` requiere deployment como API Executable, no configurado). Instrucciones de ejecución entregadas al usuario en el chat: el botón nuevo en la WebApp, o manualmente desde el editor.

**Depends on / blocked by:** ejecución manual pendiente por parte del usuario, vía el botón "Cargar Línea Cero" en `#moduloGestionEquipos` (o desde el editor de Apps Script). No depende de ningún pendiente externo adicional — a diferencia de `sincronizarGruposGoogleIDU()`, esta función solo depende de `USUARIOS` (ya sembrada en la Sección 23, pendiente de su propia ejecución) y de `Datos`, ambas siempre disponibles.

---

## 25. Sprint 6 — Desacoplamiento total de `Datos`, completitud estricta, purga de código muerto y Modo Borrador — [COMPLETADO Y DESPLEGADO 2026-08-18]

**Agente:** Claude Code (Claude Sonnet 5, orquestado con gstack v1.60.1.0).

**Origen:** el usuario confirmó éxito de la Línea Cero en producción (RTs pendientes: 9691 → 4690) pero reportó 4 problemas de UX/Performance. La ronda `/investigate` previa (ver `ARCHITECTURE_V5.md` y Sección 19 de `TODOS.md`) verificó los hallazgos y descartó la premisa "pagar/premium" (no existe ese texto en el código). Con los hallazgos confirmados, el usuario amplió el alcance del build más allá del plan original de 4 puntos (1.A–1.E) para incluir un cambio arquitectónico de fondo: desacoplar por completo la escritura de asignaciones de la hoja `Datos`.

**Cambio arquitectónico central — capa de overlay `ASIGNACIONES_EQUIPOS`:**
`Datos` pasa a ser 100% de solo lectura (nunca más se escribe ahí desde el módulo de equipos, ni siquiera por Línea Cero). Toda asignación/reasignación se escribe en una hoja nueva y dedicada `ASIGNACIONES_EQUIPOS` (columnas `RT, ARTICULADOR_EMAIL, GESTOR_EMAIL, FECHA_ACTUALIZACION, EJECUTOR`, indexada por `RT`), y el valor efectivo que ve cada consumidor es la fusión en memoria: overlay si existe, si no el valor crudo de `Datos`. Este patrón se implementó una sola vez y se reutiliza en todo el sistema:
- `gestion_equipos_backend.js` — `_leerAsignacionesEquipos()` (lee `ASIGNACIONES_EQUIPOS` completa a un mapa `{RT: {articuladorEmail, gestorEmail}}`) y `_fusionarAsignacionesConMatriz(rt, articuladorDatos, gestorDatos, asignacionesMap)` (función global, resuelve el valor efectivo por RT), consumidas también desde `Codigo.js` y `evaluador_alertas.js` aprovechando que Apps Script comparte un único scope global entre todos los archivos `.js` del proyecto (sin imports).
- `Codigo.js` — `getDashboardData()`: cada fila de cada archivo de datos se fusiona antes de aplicar el filtro RBAC y antes de empujarse a `allRecords`, así que tanto la Matriz principal como el módulo de Equipos ven siempre el valor vigente (overlay o `Datos`).
- `evaluador_alertas.js` — `ejecutarMotor()` (el motor que escribe `ALERTAS_ACTIVAS`) fusiona `ARTICULADOR JUIRIDICO`/`GESTOR JURÍDICO` antes de evaluar destinatarios de alerta. `evaluarAlertasDataset()` (ruta del dashboard en vivo) no necesitó cambios: ya consume `allRecords` de `getDashboardData()`, que llega pre-fusionado.
- El único camino de escritura a `ASIGNACIONES_EQUIPOS` es `_upsertAsignacionesEquipos(cambios, ejecutorEmail, contexto)`: lee las 4 columnas una vez, parchea en memoria, escribe con máximo 4 `setValues()` totales sin importar cuántos RT se toquen.
- `getEstadisticasCargaEquipos()`, `getProyectosConteo()`, `getTramosPorProyecto()`, `getRTsPorTramo()` se refactorizaron para leer todas desde una única función compartida `_leerFilasVisiblesRBACEquipos()` — garantiza por construcción que los badges del árbol y los KPIs nunca queden inconsistentes entre sí.

**Regla de completitud estricta (cambio de comportamiento deliberado):** un RT ahora cuenta como "sin asignar" si le falta **cualquiera** de los dos roles — `_esRTCompleto(articuladorEmail, gestorEmail) = Boolean(art && ges && art !== 'Sin asignar' && ges !== 'Sin asignar')`. Antes contaba como pendiente solo si faltaban **ambos**. Esto es lo que hace que los contadores/badges sean confiables como indicador operativo real, no solo "tiene al menos alguien".

**`asignarEquipoGranularLote(cambiosArray, ejecutorEmail)` (nuevo endpoint, backend del Modo Borrador):** bajo un único `LockService` (60s), resuelve cada entrada `{nivel, idTarget, articuladorEmail?, gestorEmail?}` vía `_resolverCambiosPorNivel()` contra una sola lectura de `Datos`, tolera errores por nodo individual sin abortar el lote completo (`erroresPorNodo`), consolida todos los cambios resultantes en un solo array y hace una única llamada a `_upsertAsignacionesEquipos()`. Devuelve `{success, totalNodosRecibidos, totalRTsAfectados, cambiosAplicados, logsRegistrados, logsConError, erroresPorNodo}`.

**Modo Borrador en `#arbolAsignacionEquipos` (`app_equipos_js.html`):** las ediciones del modal de asignación granular ya no llaman a `asignarEquipoGranular()` directo (round-trip + lock por cada click) — `agregarCambioAlBorrador()` (antes `handleAsignarEquipoGranular()`) escribe en un estado local `borradorCambiosEquipos` (clave por nodo: `P::proyecto`, `T::proyecto::tramo`, `R::proyecto::tramo::rt`) y repinta el árbol de inmediato con resaltado optimista (`border-warning`/`table-warning`, badge "Cambio pendiente", icono de punto pendiente por RT). Un RT hereda el borrador más específico que lo cubra (RT propio → TRAMO → PROYECTO), igual que la resolución en cascada del servidor. Reabrir el modal sobre un nodo con borrador precarga sus valores para seguir editando en vez de partir de cero. Una barra flotante (`ensureBarraBorradorEquipos()`, inyectada dinámicamente, mismo patrón que el modal de homologación) muestra el conteo de cambios pendientes con botones "Descartar" y "Guardar Todos los Cambios"; este último arma el `cambiosArray` y hace la única llamada batch a `asignarEquipoGranularLote()`.

**Purga de código muerto (Admin SDK / Google Groups):** `homologacion_usuarios.js` pasó de 543 a 312 líneas — se eliminaron `_obtenerMiembrosGrupo()`, `_obtenerNombreCompleto()`, `_obtenerDirectorioGruposIDU()`, `sincronizarGruposGoogleIDU()` y las constantes `GRUPOS_OFICIALES_IDU`/`GRUPOS_DIRECTORIO_CACHE_KEY`/`GRUPOS_DIRECTORIO_CACHE_TTL`; `_leerDirectorioCombinado()` vuelve a ser una lectura directa de `USUARIOS` (`_leerDirectorioUsuariosSheet()`). `appsscript.json` revirtió a los scopes previos a Admin SDK (sin servicio avanzado `AdminDirectory`, sin sus 2 scopes OAuth) — reduce la superficie de permisos solicitados al usuario en el consentimiento de autorización. El estado `ENCONTRADO_SIN_PERFIL` queda como código defensivo intencionalmente muerto (documentado en el docstring del archivo), ya que dependía del directorio de Grupos ahora retirado. La Línea Cero (Sección 24) queda como la solución final aceptada para el problema que Admin SDK intentaba resolver.

**Otros fixes puntuales del mismo build:**
- `Index.html`: eliminado `<?!= include('export_backend') ?>` — `export_backend.js` es un archivo de servidor plano V8, nunca debió pasar por `include()` (esa función solo resuelve parciales `.html`).
- `app_core_js.html`: `refreshData()` ahora llama a `invalidateDataCache()` en el servidor **y** purga ambos stores de IndexedDB (`dashboard-cache-v1`/`dashboard-cache-store` y `equipos-cache-v1`/`stats`, clave por `CURRENT_USER_EMAIL`) antes de recargar — antes el botón "Recargar" solo disparaba `loadDashboardData()`, dejando dos capas de datos obsoletos (caché de servidor + IndexedDB local) sin purgar.
- `app_matriz_js.html`: `filterData()` ahora parte de `getBaseFilteredData()` (que ya aplicaba el recorte de `window.filtroMatrizActivoGlobal` pero cuyo resultado la tabla ignoraba) en vez de `rawData` directo. `aplicarFiltroAdhoc()` filtra al instante en memoria sobre los datos ya cargados y persiste la preferencia en `FiltroMatriz` en segundo plano, sin bloquear el repintado. `restablecerFiltroAdhoc()` se deja con round-trip real al servidor de forma deliberada: el recorte en memoria solo puede *estrechar* lo que el cliente ya tiene cargado (el servidor sigue pre-filtrando por el `FiltroMatriz` activo antes de enviar el payload), nunca puede *ensanchar* de vuelta a proyectos que nunca llegaron — limitación conocida y documentada en el propio código, no resuelta por diseño.

**Verificación ejecutada:** `node --check` sobre `config.js`, `gestion_equipos_backend.js`, `homologacion_usuarios.js`, `Codigo.js`, `evaluador_alertas.js` — 5/5 OK. `JSON.parse` sobre `appsscript.json` — OK. Extracción + `node --check` de los bloques `<script>` de `app_matriz_js.html`, `app_equipos_js.html` y `app_core_js.html` — 3/3 OK (el segundo bloque `<script>` de `Index.html` no se puede parsear con Node porque contiene un scriptlet GAS `<?!= ... ?>`, esperado y no relacionado con este cambio). Grep dirigido confirmó cero referencias restantes a `AdminDirectory`/`sincronizarGruposGoogleIDU`/`GRUPOS_OFICIALES_IDU` fuera del comentario explicativo, y cero referencias a `include('export_backend')`.
